...
function fmtEvent(row) {
  return {
    ...
    location: {
      ...
      city:      row.location_city || 'Kampala',
...
router.post('/otp/send', async (req, res) => {
  ...
  try {
    ...
    await sendSMS(phone, `Your REDE code is: ${code}. Valid 10 minutes.`)
  ...
  try {
    ...
    if (!userRows[0]) {
      const { rows } = await query(
        'INSERT INTO users (phone, verified) VALUES ($1, TRUE) RETURNING *', [phone]
      )
      userRows  = rows
      isNewUser = true
    } else {
...
router.put('/profile', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (nickname && nickname !== req.user.name && nickname !== req.user.nickname) {
      const { rows: logRows } = await query(
        'SELECT changed_at FROM name_change_log WHERE user_id=$1', [req.user.id]
      )
      if (logRows[0]) {
        const daysSince = (Date.now() - new Date(logRows[0].changed_at)) / 86400000
        if (daysSince < 7) {
          const left = Math.ceil(7 - daysSince)
          return res.status(400).json({ message: `Nickname can be changed again in ${left} day${left > 1 ? 's' : ''}` })
        }
      }
      await query(
        'INSERT INTO name_change_log (user_id, changed_at) VALUES ($1,NOW()) ON CONFLICT (user_id) DO UPDATE SET changed_at=NOW()',
        [req.user.id]
      )
    }
...
router.get('/organizer/:id', async (req, res) => {
  try {
    const { rows: userRows } = await query(
      'SELECT id, name, nickname, first_name, last_name, avatar_url, verified, created_at FROM users WHERE id=$1',
      [req.params.id]
    )
    ...
    const u         = userRows[0]
    const avgRating = eventRows.length > 0
...
router.get('/search-organisers', async (req, res) => {
  ...
  try {
    const { rows } = await query(`
      SELECT u.id, u.name, u.nickname, u.avatar_url AS avatar, u.verified,
             COUNT(DISTINCT e.id) AS event_count,
             ROUND(AVG(r.rating), 1) AS avg_rating
      FROM users u
      LEFT JOIN events e ON e.organizer_id = u.id AND e.is_active = TRUE
      LEFT JOIN reviews r ON r.event_id = e.id
      WHERE u.name ILIKE $1 OR u.nickname ILIKE $1
      GROUP BY u.id
