...
router.get('/:id/attendees', async (req, res) => {
  ...
  try {
    const { rows } = await query(`
      SELECT u.id, u.nickname, u.name, u.avatar_url AS avatar, u.verified
      FROM attendees a
...
router.put('/:id', requireAuth, async (req, res) => {
  try {
    ...
    const {
      ...
      start_time, end_time, cover_image, location,
      entry_fee,
    } = req.body

    // Build location columns
    const locName    = location?.venueName || location?.name || null
    const locAddress = location?.area      || locName
    const locLat     = location?.lat       ? parseFloat(location.lat) : null
    const locLng     = location?.lng       ? parseFloat(location.lng) : null
    const locCity    = location?.city      || null
    const locArea    = location?.area      || null

    const { rows } = await query(`
      UPDATE events SET
        title           = COALESCE($1,  title),
        description     = COALESCE($2,  description),
        max_attendees   = COALESCE($3,  max_attendees),
        ...
        location_area   = COALESCE($13, location_area)
    ...
    `, [
      ...
      end_time         || null,
      cover_image      || null,
      entry_fee != null ? parseInt(entry_fee) : null,
      locName,
      locAddress,
      locLat,
      locLng,
